param([string]$ScriptName = 'MMEAlertsQuickTest')
...
function Make-Subrecord([string]$signature, [byte[]]$data) {
...
function Make-Entry([string]$name) {
...
function Join-Bytes($parts) {
    ...
    $offset = 0
    foreach ($part in $parts) { $part.CopyTo($result, $offset); $offset += $part.Length }
    $result
}
    ...
    if ($label -eq 'QUST') {
        ...
        while ($cursor -lt $dataEnd) {
            ...
            if ($signature -eq 'VMAD') {
                ...
                $parts.Add((Make-Subrecord 'VMAD' $newVmad))
            } else {
...
$writeAt = 24 + $headerSize
foreach ($group in $groups) { $group.CopyTo($output, $writeAt); $writeAt += $group.Length }
[IO.File]::WriteAllBytes($pluginPath, $output)
Write-Host "Attached $ScriptName to the MME Alerts quest."
