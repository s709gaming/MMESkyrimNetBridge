...
$mParts=[Collections.Generic.List[byte[]]]::new();$mParts.Add((Sub 'EDID' ([Text.Encoding]::ASCII.GetBytes("MMEAlerts_PlayerMonitorEffect`0"))));$mParts.Add((Sub 'VMAD' $vm));$data=New-Object byte[] 152;$mParts.Add((Sub 'DATA' $data));$mParts.Add((Sub 'SNDD' (New-Object byte[] 0)));$mgef=NewGroup 'MGEF' (Record 'MGEF' 0x01000804 (Join $mParts))

$tmpl=[IO.File]::ReadAllBytes((Join-Path $root 'tools\ability-template.bin'));$sParts=[Collections.Generic.List[byte[]]]::new();$k=0;while($k-lt$tmpl.Length){$s=[Text.Encoding]::ASCII.GetString($tmpl,$k,4);$l=U16 $tmpl ($k+4);if($s-eq'EDID'){$sParts.Add((Sub 'EDID' ([Text.Encoding]::ASCII.GetBytes("MMEAlerts_PlayerMonitorAbility`0"))))}elseif($s-ne'EFID'-and$s-ne'EFIT'-and$s-ne'FULL'-and$s-ne'DESC'){$c=New-Object byte[](6+$l);[Array]::Copy($tmpl,$k,$c,0,$c.Length);$sParts.Add($c)};$k+=6+$l};$efid=New-Object byte[] 4;W32 $efid 0 0x01000804;$efit=New-Object byte[] 12;$sParts.Add((Sub 'EFID' $efid));$sParts.Add((Sub 'EFIT' $efit));$spel=NewGroup 'SPEL' (Record 'SPEL' 0x01000805 (Join $sParts))
