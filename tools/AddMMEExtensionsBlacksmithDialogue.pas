...
{
  Adds two state-exclusive Blacksmith choices under MME's existing
...
const
  ...
  VendorConditionInfoFormID = $0008069E;
  ...
  StateGlobalEditorID = 'MMEExt_BlacksmithArmorState';
  AddTopicEditorID = 'MMEExt_BlacksmithAddMilkArmorTopic';
  AddInfoEditorID = 'MMEExt_BlacksmithAddMilkArmor';
  RemoveTopicEditorID = 'MMEExt_BlacksmithRemoveMilkArmorTopic';
  RemoveInfoEditorID = 'MMEExt_BlacksmithRemoveMilkArmor';

  HandlerScriptName = 'MMEBlacksmithDialogue';
  StatePropertyName = 'MMEExt_BlacksmithArmorState';
  RefreshFragmentName = 'Fragment_RefreshBlacksmithArmorState';
  AddFragmentName = 'Fragment_AddMilkArmor';
  RemoveFragmentName = 'Fragment_RemoveMilkArmor';

  AddPrompt = 'Could you fit this armor for milking?';
  AddResponse = 'You Milk Maids do plenty for folks around here. I''ll fit the collection pieces. On the house.';
  RemovePrompt = 'Could you remove the milking fittings?';
  RemoveResponse = 'I''ll strip the fittings off and leave the armor as it was. No charge for a milk maid.';
...
var
  ...
  JobBlacksmithFaction, JobMerchantFaction: IInterface;
  BlacksmithCondition, MerchantCondition: IInterface;
...
begin
  if not Assigned(aElement) then
    Exit;
  if Assigned(BlacksmithCondition) and Assigned(MerchantCondition) and
      ...
      for j := 0 to ElementCount(conditions) - 1 do begin
        ...
        if SameText(functionName, 'GetInFaction') and
          ...
          if Equals(MasterOrSelf(linked),
              MasterOrSelf(JobBlacksmithFaction)) then begin
            if not Assigned(BlacksmithCondition) then
              BlacksmithCondition := condition;
          end
...
begin
  ...
  if not Assigned(StateGlobal) then begin
    AddMessage('ERROR: Could not create the Blacksmith state Global.');
    Exit;
  end;
...
begin
  ...
  if not Assigned(conditions) then
    Exit;
  if not AppendCondition(conditions, BlacksmithCondition) or
...
var
  ...
  openingLinks, vmad, vendorConditionInfo, mmeTradeInnTopic: IInterface;
  linksBefore, linksExpected: Integer;
begin
  Result := 1;
  AddMessage('MME Extensions Blacksmith installer revision 6 (OG MME service conditions).');
  ...
  JobBlacksmithFaction := FindRecordByEditorIDRecursive(
    GroupBySignature(SkyrimFile, 'FACT'), 'FACT',
    JobBlacksmithFactionEditorID);
  JobMerchantFaction := FindRecordByEditorIDRecursive(
    GroupBySignature(SkyrimFile, 'FACT'), 'FACT',
    JobMerchantFactionEditorID);
  if not Assigned(JobBlacksmithFaction) or
     not Assigned(JobMerchantFaction) then begin
    AddMessage('ERROR: Skyrim job factions were not found.');
    Exit;
  end;

  { Follow OG MME's service-dialogue convention: its Inn trade topic supplies
    JobMerchantFaction; vanilla INFO 0008069E supplies the missing
    JobBlacksmithFaction shape. MME does not gate these services with
    GetOffersServicesNow. SourceInfo supplies GetGlobalValue. }
  mmeTradeInnTopic := FindRecordByEditorIDRecursive(
    GroupBySignature(MMEFile, 'DIAL'), 'DIAL', MMETradeInnTopicEditorID);
  vendorConditionInfo := RecordByFormID(SkyrimFile,
    VendorConditionInfoFormID, True);
  if not Assigned(mmeTradeInnTopic) or not Assigned(vendorConditionInfo) or
     (Signature(vendorConditionInfo) <> 'INFO') then begin
    AddMessage('ERROR: Verified MME/Skyrim service-condition templates were not found.');
    Exit;
  end;
  FindConditionTemplates(SourceInfo);
  FindConditionTemplates(mmeTradeInnTopic);
  FindConditionTemplates(vendorConditionInfo);
  if not Assigned(BlacksmithCondition) or
  ...
  AddMessage('MME Extensions Blacksmith dialogue installed successfully.');
