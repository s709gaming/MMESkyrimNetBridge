...
{
  Adds two state-exclusive Alchemist Living Armor choices under MME's existing
...
const
  ...
  JobApothecaryFactionEditorID = 'JobApothecaryFaction';
  ...
  StateGlobalEditorID = 'MMEExt_AlchemistLivingArmorState';
  AddTopicEditorID = 'MMEExt_AlchemistAddLivingArmorTopic';
  AddInfoEditorID = 'MMEExt_AlchemistAddLivingArmor';
  RemoveTopicEditorID = 'MMEExt_AlchemistRemoveLivingArmorTopic';
  RemoveInfoEditorID = 'MMEExt_AlchemistRemoveLivingArmor';

  HandlerScriptName = 'MMEBlacksmithDialogue';
  StatePropertyName = 'MMEExt_AlchemistLivingArmorState';
  RefreshFragmentName = 'Fragment_RefreshBlacksmithArmorState';
  AddFragmentName = 'Fragment_AddLivingArmor';
  RemoveFragmentName = 'Fragment_RemoveLivingArmor';

  AddPrompt = 'I''m looking for something kinky to wear under my armor. Something sensuous that wraps around me.';
  AddResponse = 'I know just the thing. A naughty little creature, very affectionate, and surprisingly good at helping a Milk Maid find relief.';
  RemovePrompt = 'Could you get this little creature out of my armor?';
  RemoveResponse = 'Had enough of your little friend already? Shame. It seemed rather attached to you.';
...
var
  ...
  JobApothecaryFaction, JobMerchantFaction: IInterface;
  ApothecaryCondition, MerchantCondition: IInterface;
...
begin
  if not Assigned(aElement) then
    Exit;
  if Assigned(ApothecaryCondition) and Assigned(MerchantCondition) and
      ...
      for j := 0 to ElementCount(conditions) - 1 do begin
        ...
        if SameText(functionName, 'GetInFaction') and
          ...
          if Equals(MasterOrSelf(linked),
              MasterOrSelf(JobApothecaryFaction)) then begin
            if not Assigned(ApothecaryCondition) then
              ApothecaryCondition := condition;
          end
...
begin
  ...
  if not Assigned(StateGlobal) then begin
    AddMessage('ERROR: Could not create the Alchemist state Global.');
    Exit;
  end;
...
begin
  ...
  if not Assigned(conditions) then
    Exit;
  if not AppendCondition(conditions, ApothecaryCondition) or
...
begin
  Result := 1;
  AddMessage('MME Extensions Alchemist installer revision 6 (OG MME service conditions).');
  ...
  JobApothecaryFaction := FindRecordByEditorIDRecursive(
    GroupBySignature(SkyrimFile, 'FACT'), 'FACT',
    JobApothecaryFactionEditorID);
  JobMerchantFaction := FindRecordByEditorIDRecursive(
    GroupBySignature(SkyrimFile, 'FACT'), 'FACT',
    JobMerchantFactionEditorID);
  if not Assigned(JobApothecaryFaction) or
     not Assigned(JobMerchantFaction) then begin
    AddMessage('ERROR: Skyrim job factions were not found.');
    Exit;
  end;

  { Follow OG MME's service-dialogue convention. Its own merchant topics
    supply both JobMerchantFaction and JobApothecaryFaction condition rows;
    MME does not gate these services with GetOffersServicesNow. SourceInfo
    supplies the extension GetGlobalValue row. }
  ...
  FindConditionTemplates(GroupBySignature(MMEFile, 'DIAL'));
  if not Assigned(ApothecaryCondition) or
  ...
  AddMessage('MME Extensions Alchemist dialogue installed successfully.');
